...
- [txguard – Solana tx pre-sign drain-check](https://txguard-985332749804.europe-north1.run.app) — x402-payable safety gate: `POST /v1/txguard` with a decoded tx → HTTP 402 → pay 0.01 USDC on Solana → re-call with `?pay=<sig>` → verdict allow|warn|halt (catches full-drain, setAuthority, unlimited-delegate, unknown-program). Free demo at `/v1/txguard/demo`. No account, no KYC, deterministic.
