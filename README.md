...
- [anchor-x402-mcp](https://www.npmjs.com/package/anchor-x402-mcp) - 14-tool MCP server for [anchor-x402](https://anchor-x402.com) on Base + Solana. **9 commodity primitives** (anchor, screen, attest, decode/tx, resolve/name, price/token, decode/calldata, parse/datetime, intel/wallet) plus **5 universal LLM endpoints** (roast, oracle with on-chain anchored verdict, tldr, aura, grade) — added in v0.2.0. Auto-pays from the agent's Base wallet via x402-fetch + viem. No prepay, no API key, no account. Works with Claude Desktop, Claude Code, Codex CLI, ChatGPT Desktop, Cursor, OpenAI Agents SDK, and any MCP-compatible client. `npx -y anchor-x402-mcp`. ([GitHub](https://github.com/hypeprinter007-stack/anchor-x402-mcp))
